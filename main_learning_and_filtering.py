...
def initialize_true_price_models(T, N_assets, sigma_noise_impact, sigma_noise_price, test='Almgren_and_Chriss'):
    ...
    for _ in range(T):
        ...
        if test == 'Almgren_and_Chriss':
            ...
            A_theta_true_torched = torch.tensor(A_theta_true)
            A_eta_true = utils.construct_decay_matrix(X_0=X_eta_0, decay_type='heterogeneous') * 1.e-2
            A_eta_true_torched = torch.tensor(A_eta_true)
...
def initialize_kalman_filters(N_assets, test='Almgren_and_Chriss'):
    ...
    if test == 'Almgren_and_Chriss':
        ...
        A_theta_init_torched = torch.eye(N_assets**2)*1.e-2
        A_eta_init_torched = torch.eye(N_assets**2)*2.e-2
        ...
        A_theta_init_torched = torch.tensor(A_theta_init)
        A_eta_init = utils.construct_decay_matrix(X_0=X_eta_0, decay_type='heterogeneous')
        A_eta_init_torched = torch.tensor(A_eta_init)
...
def main_simulation(T=2, K=100, N_assets=2, q0=1, sigma_noise_impact=1.e-5, sigma_noise_price = 1.e-5):
    ...
    KF_permanent, KF_temporary = initialize_kalman_filters(N_assets, 'Almgren_and_Chriss')
